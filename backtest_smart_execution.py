"""
PHASE 9: SMART EXECUTION - TURN ALPHA INTO PROFIT
==================================================

THE PROBLEM (Phase 8):
  - Win Rate: 57-62% (EXCELLENT!)
  - But: -6.08% loss due to transaction costs
  - Root Cause: Too many trades (657) with low magnitude returns

THE SOLUTION:
  1. VOLATILITY FILTER: Don't trade when movement < commission
  2. CUMULATIVE HOLD: Ride trends, don't churn
  3. SMART EXIT: Only exit when model reverses

Strategy Changes:
  OLD (Phase 8):
    - Every hour: If signal=1 -> BUY -> SELL next hour
    - 657 trades, -6.08% return

  NEW (Phase 9):
    - Volatility Filter: Only trade if ATR > 0.2% of price
    - Buy and HOLD while model says UP
    - Only sell when model says DOWN
    - Expected: Far fewer trades, higher profit per trade

Win Rate is GOLD. Now let's make it PROFITABLE.
"""
...
RESULTS_DIR = PROJECT_ROOT / "data" / "backtest_smart"
...
def add_intraday_features(df):
    ...
    # ATR (Average True Range) - CRITICAL FOR SMART EXECUTION
...
def backtest_stock_smart(ticker):
    """
    Backtest with SMART EXECUTION strategy.

    Strategy:
      1. VOLATILITY FILTER: Only trade if ATR > 0.2% of price
      2. CUMULATIVE HOLD: Hold position while signal stays UP
      3. SMART EXIT: Only exit when signal flips to DOWN

    Returns:
      dict with performance metrics
    """
    print(f"\n{'='*80}")
    print(f"BACKTESTING (SMART): {ticker}")
    print(f"{'='*80}")
    ...
    # Track volatility filtering
    total_signals = 0
    filtered_signals = 0

    for i in range(len(test_df)):
        current_idx = test_df.index[i]
        signal = test_df.loc[current_idx, 'signal']
        confidence = test_df.loc[current_idx, 'confidence']
        current_close = test_df.loc[current_idx, 'close']
        atr_pct = test_df.loc[current_idx, 'ATR_pct']

        # SMART EXECUTION LOGIC
        if not position_open:
            # Looking to ENTER
            if signal == 1:  # Model says UP
                total_signals += 1

                # VOLATILITY FILTER: Is expected move > transaction cost?
                if atr_pct < MIN_ATR_THRESHOLD:
                    filtered_signals += 1
                    continue  # Skip this trade - not enough volatility
            ...
            # Position is OPEN - should we HOLD or EXIT?
            if signal == 0:  # Model says DOWN - EXIT
                # SELL at current close
                sell_proceeds = shares * current_close
            ...
            # else: signal == 1 -> HOLD POSITION (do nothing)

    # Close any remaining position at end of test period
    if position_open:
        final_close = test_df.iloc[-1]['close']
        sell_proceeds = shares * final_close
        sell_cost = sell_proceeds * TRANSACTION_COST
        sell_proceeds -= sell_cost

        profit = sell_proceeds - capital
        profit_pct = profit / capital
        hold_duration = len(test_df) - 1 - entry_idx
    ...
    avg_hold_duration = trades_df['hold_duration_hours'].mean()

    # Print results
    print(f"\n  SMART EXECUTION STATS:")
    print(f"    Total UP signals:      {total_signals}")
    print(f"    Filtered (low ATR):    {filtered_signals} ({filtered_signals/total_signals*100 if total_signals > 0 else 0:.1f}%)")
    print(f"    Trades executed:       {len(trades_df)}")
    print(f"    Avg hold duration:     {avg_hold_duration:.1f} hours")
    ...
    # Save trade log
    trades_df.to_csv(RESULTS_DIR / f"{ticker}_smart_trades.csv", index=False)
...
def run_smart_backtest():
    """Run smart execution backtest for all winner stocks"""
    print("="*80)
    print("PHASE 9: SMART EXECUTION - TURN ALPHA INTO PROFIT")
    print("="*80)
    ...
    print(f"  1. VOLATILITY FILTER: ATR must be > {MIN_ATR_THRESHOLD:.2%} of price")
    print(f"  2. CUMULATIVE HOLD: Hold while model says UP")
    print(f"  3. SMART EXIT: Only exit when model says DOWN")

    # Backtest each stock
    results = []
    for ticker in WINNER_STOCKS:
        try:
            result = backtest_stock_smart(ticker)
    ...
    # Portfolio Summary
    print(f"\n{'='*80}")
    print("PORTFOLIO SUMMARY - SMART EXECUTION")
    print(f"{'='*80}")
    ...
    avg_hold_duration = results_df['avg_hold_duration'].mean()
    total_signals = results_df['total_signals'].sum()
    total_filtered = results_df['filtered_signals'].sum()
    ...
    print(f"    Filtered (ATR):     {total_filtered} ({total_filtered/total_signals*100 if total_signals > 0 else 0:.1f}%)")
    ...
    print(f"{'='*80}")
    print(f"\n  Phase 8 (Naive):     657 trades, -6.08% return, $9,391.59 final")
    print(f"  Phase 9 (Smart):     {total_trades} trades, {portfolio_return:+.2%} return, ${portfolio_final_value:,.2f} final")
    print(f"")
    print(f"  Trade Reduction:     {(657-total_trades)/657*100:.1f}%")
    print(f"  Return Improvement:  {(portfolio_return - (-0.0608))*100:+.2f} percentage points")

    # Save summary
    results_df.to_csv(RESULTS_DIR / "smart_backtest_summary.csv", index=False)
    ...
    portfolio_summary = {
        ...
        'avg_hold_duration': avg_hold_duration,
        'total_signals': total_signals,
        'filtered_signals': total_filtered
    }

    portfolio_df = pd.DataFrame([portfolio_summary])
    portfolio_df.to_csv(RESULTS_DIR / "smart_portfolio_summary.csv", index=False)

    print(f"\n{'='*80}")
    print("BACKTEST COMPLETE")
    print(f"{'='*80}")
    print(f"\nResults saved:")
    print(f"  - Individual trades: {RESULTS_DIR}/[TICKER]_smart_trades.csv")
    print(f"  - Stock summary:     {RESULTS_DIR}/smart_backtest_summary.csv")
    print(f"  - Portfolio summary: {RESULTS_DIR}/smart_portfolio_summary.csv")
    ...
    if portfolio_return > 0.10:  # >10% return
        print(f"  [OUTSTANDING] Portfolio returned {portfolio_return:.2%}")
        print(f"  Smart execution turned Alpha into PROFIT!")
    elif portfolio_return > 0.05:  # >5% return
        print(f"  [SUCCESS] Portfolio returned {portfolio_return:.2%}")
        print(f"  System is PROFITABLE with smart execution.")
    elif portfolio_return > 0:
        print(f"  [MARGINAL] Portfolio returned {portfolio_return:.2%}")
        print(f"  Profitable but needs further optimization.")
    else:
        print(f"  [FAILED] Portfolio lost {abs(portfolio_return):.2%}")
        print(f"  Even smart execution couldn't overcome costs.")

    print(f"\n  Bottom Line: If you invested ${STARTING_CAPITAL:,.2f} using SMART execution,")
